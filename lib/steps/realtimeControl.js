  ...
  run(callback) {
    ...
    for (i = 0; i < this.config.documents.packetSize - 1; i++) {
      this.buffer.clients[(this.done + i) % this.buffer.clients.length]
        .dataCollectionFactory(this.config.kuzzle.collection)
    ...
    this.buffer.clients[this.done  % this.buffer.clients.length]
      ...
      .publishMessage(this.buffer.documents[(this.done + 1) % this.buffer.documents.length], (err, response) => {
        ...
        const meanElapsed = this.elapsedList.reduce((p, c) => c + p, 0) / this.elapsedList.length;
        const meanMessagesSecond = this.config.documents.packetSize / meanElapsed;
        ...
        this.stdout && logUpdate(`${Math.round(this.done*100/this.config.documents.count)}% | Elapsed: ${Math.round(this.totalTime)}s | ${Math.round(meanMessagesSecond)} msg/s | Mean packet time: ${meanElapsed.toFixed(2)}s`);
