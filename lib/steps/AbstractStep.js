...
class AbstractStep {
  ...
  benchmark(fn, callback, opts = {}) {
    ...
    const
      ...
      timer = setInterval(() => {
        ...
        this.log.update(`${Math.round(total*100/batchSize)}% | Requests completed: ${total} | Elapsed: ${Math.round(elapsed / 1000)}s | ${requestsSecond} requests/s | ${notifications} notifications received`);
  ...
  unsubscribe(callback) {
    const
      ...
      currySubscribe = idx => {
        return cb => {
          this.buffer.clients[idx].benchmarkListener = payload => {
